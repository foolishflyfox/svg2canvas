    ...
    function build(opts) {
      ...
      svg.AspectRatio = function (
        ...
        if (refX.hasValue() && refY.hasValue()) {
          ...
          if (
            ...
            ctx.translate2(width / 2.0 - desiredWidth / 2.0, 0);
          if (
            align.match(/YMid$/) &&
            ((meetOrSlice == "meet" && scaleMin == scaleX) ||
              (meetOrSlice == "slice" && scaleMax == scaleX))
          )
            ctx.translate2(0, height / 2.0 - desiredHeight / 2.0);
          if (
            align.match(/^xMax/) &&
            ((meetOrSlice == "meet" && scaleMin == scaleY) ||
              (meetOrSlice == "slice" && scaleMax == scaleY))
          )
            ctx.translate2(width - desiredWidth, 0);
          if (
            align.match(/YMax$/) &&
            ((meetOrSlice == "meet" && scaleMin == scaleX) ||
              (meetOrSlice == "slice" && scaleMax == scaleX))
          )
            ctx.translate2(0, height - desiredHeight);
        }
      ...
      svg.Element.PathElementBase = function (node) {
        ...
        this.renderChildren = function (ctx) {
          ...
          if (ctx.fillStyle2 != "") {
            if (
              this.style("fill-rule").valueOrDefault("inherit") != "inherit"
            ) {
        ...
        this.setContext = function (ctx) {
          ...
          if (
            ...
            var fontSize = new svg.Property(
              "fontSize",
              svg.Font.Parse(ctx.font2).fontSize
            );
            ...
            switch (pp.command) {
              ...
              case "q":
                while (!pp.isCommandOrEnd()) {
                  ...
                  if (ctx != null)
                    ctx.quadraticCurveTo2(cntrl.x, cntrl.y, cp.x, cp.y);
                }
                break;
              case "T":
              case "t":
      ...
      svg.Element.marker = function (node) {
        ...
        this.render = function (ctx, point, angle) {
          ...
          if (
            ...
            ctx.scale2(ctx.lineWidth2, ctx.lineWidth2);
          ctx.save2();
          ...
          if (
            ...
            ctx.scale2(1 / ctx.lineWidth2, 1 / ctx.lineWidth2);
          if (this.attribute("orient").valueOrDefault("auto") == "auto")
            ctx.rotate2(-angle);
          ctx.translate2(-point.x, -point.y);
        };
        ...
        this.renderChildren = function (ctx) {
          ...
          if (customFont != null) {
            var fontSize = this.parent
              .style("font-size")
              .numValueOrDefault(svg.Font.Parse(svg.ctx.font2).fontSize);
            var fontStyle = this.parent
              .style("font-style")
              .valueOrDefault(svg.Font.Parse(svg.ctx.font2).fontStyle);
            ...
            for (var i = 0; i < text.length; i++) {
              ...
              ctx.lineWidth2 =
                (ctx.lineWidth2 * customFont.fontFace.unitsPerEm) / fontSize;
              if (fontStyle == "italic") ctx.transform2(1, 0, 0.4, 1, 0, 0);
              glyph.render(ctx);
              if (fontStyle == "italic") ctx.transform2(1, 0, -0.4, 1, 0, 0);
              ctx.lineWidth2 = lw;
          ...
          if (ctx.paintOrder == "stroke") {
            ...
            if (ctx.fillStyle2 != "")
              ctx.fillText2(svg.compressSpaces(this.getText()), this.x, this.y);
          } else {
            if (ctx.fillStyle2 != "")
              ctx.fillText2(svg.compressSpaces(this.getText()), this.x, this.y);
            if (ctx.strokeStyle2 != "")
      ...
      svg.Element.a = function (node) {
        ...
        this.renderChildren = function (ctx) {
          if (this.hasText) {
            ...
            var fontSize = new svg.Property(
              "fontSize",
              svg.Font.Parse(svg.ctx.font2).fontSize
            );
      ...
      svg.Element.textPath = function (node) {
        ...
        this.renderChildren = function (ctx) {
          ...
          for (var i = 0; i < glyphInfo.length; i++) {
            ...
            if (ctx.fillStyle2 != "")
              ctx.fillText2(svg.compressSpaces(partialText), 0, 0);
            if (ctx.strokeStyle2 != "")
              ctx.strokeText2(svg.compressSpaces(partialText), 0, 0);
            ctx.restore2();
            if (textDecoration === "underline") {
              if (i === 0) {
        ...
        this.path = function (ctx) {
          ...
          for (var n = 0; n < ca.length; n++) {
            ...
            switch (c) {
              ...
              case "C":
                if (ctx != null)
                  ctx.bezierCurveTo2(p[0], p[1], p[2], p[3], p[4], p[5]);
                break;
              case "Q":
        ...
        this.setTextData = function (ctx) {
          ...
          var startOffset =
            this.style("startOffset").numValueOrDefault(0) * fullPathWidth;
          var offset = 0;
      ...
      svg.Element.use = function (node) {
        ...
        this.setContext = function (ctx) {
          this.baseSetContext(ctx);
          if (this.attribute("x").hasValue())
            ctx.translate2(this.attribute("x").toPixels("x"), 0);
          if (this.attribute("y").hasValue())
            ctx.translate2(0, this.attribute("y").toPixels("y"));
        };
          ...
          svg.intervalID = setInterval(function () {
            ...
            if (svg.opts["ignoreAnimation"] != true) {
              for (var i = 0; i < svg.Animations.length; i++) {
                var needAnimationUpdate = svg.Animations[i].update(
                  1000 / svg.FRAMERATE
                );
      ...
      svg.Mouse = new (function () {
        ...
        this.checkPath = function (element, ctx) {
          for (var i = 0; i < this.events.length; i++) {
            var e = this.events[i];
            if (ctx.isPointInPath && ctx.isPointInPath2(e.x, e.y))
